...
# Note
# var Note = load("res://assets/")
# var RedMaterial = load("res://theme/colors/")
# var BlueMaterial = load("res://theme/colors/songs/")

# Song
var current_song = "./exampleCalibration/expert.dat"
var song_data = {}
var notes_on_track = []
var batches = []
...
func spawn_note(note):
	...
	match current_direction:
		...
		NOTE_DIRECTION.DownRight:
			new_note.rotate_z(deg2rad(45.0))
		NOTE_DIRECTION.All:
			new_note.get_node('direction').visible = false
		
	if current_direction != NOTE_DIRECTION.All:
		new_note.get_node('allDriections').visible = false
	
	if note['_type'] == 0: # 0 Red, 1 Blue
		
		new_note.get_node('direction/CSGPolygon').material_override = redMaterial
		new_note.get_node('allDriections/CSGPolygon').material_override = redMaterial
	else:
		new_note.get_node('direction/CSGPolygon').material_override = blueMaterial
		new_note.get_node('allDriections/CSGPolygon').material_override = blueMaterial
